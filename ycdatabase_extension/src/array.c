...
//单元素数组重组
void yc_array_single_columns(zval** return_single_column_result, zval* data) {
	...
	YC_HASHTABLE_FOREACH_START2(Z_ARRVAL_P(data), key, key_len, key_type, value)
		...
		uint32_t key_len2;
		int key_type2;
		YC_HASHTABLE_FOREACH_START2(Z_ARRVAL_P(value), key2, key_len2, key_type2, value2)
			zval *copy = yc_zval_copy(value2);
